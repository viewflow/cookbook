...
@tag("integration")
@override_settings(ROOT_URLCONF=__name__[:-11] + "config.urls")
class Test(TestCase):  # noqa: D101
    ...
    def setUp(self):  # noqa: D102
        ...
        self.city = models.City.objects.create(
            name="Omsk", is_capital=False, population="1100000", country=self.country
        )
    ...
    def test_index_redirect(self):  # noqa: D102
        response = self.client.get(reverse("atlas:index"))
        self.assertRedirects(response, reverse("atlas:city:index"))

    def test_ocean_list_view(self):  # noqa: D102
        response = self.client.get(reverse("atlas:ocean:index"))
        self.assertEqual(response.status_code, 200)

    def test_sea_list_view(self):  # noqa: D102
        # count & select list + session & user + 2 queries for filter options + Ocean default value
        with self.assertNumQueries(7):
            response = self.client.get(reverse("atlas:sea:index"))
        self.assertEqual(response.status_code, 200)

    def test_sea_add_view(self):  # noqa: D102
        test_data = {
            "name": "Baltic Sea",
            "ocean": self.ocean.pk,
            "area": 138600,
    ...
    def test_sea_change_view(self):  # noqa: D102
        test_data = {
            "name": "Baltic Sea",
            "ocean": self.ocean.pk,
            "area": 138600,
        ...
        response = self.client.post(
            reverse("atlas:sea:change", args=[self.sea.pk]), test_data
        )
        self.assertEqual(response.status_code, 302)
        models.Sea.objects.get(pk=self.sea.pk, **test_data)

    def test_sea_delete_view(self):  # noqa: D102
    ...
    def test_continent_list_view(self):  # noqa: D102
        response = self.client.get(reverse("atlas:continent:index"))
        self.assertEqual(response.status_code, 200)

    def test_continent_add_view(self):  # noqa: D102
        test_data = {
            "name": "Antarctica",
            "area": 30370000,
            "population": 11000000000,
        ...
        self.assertEqual(
            list(continent.oceans.values_list("pk", flat=True)), [self.ocean.pk]
        )

    def test_continent_change_view(self):  # noqa: D102
        test_data = {
            "name": "Antarctica",
            "area": 30370000,
            "population": 11000000000,
        ...
        response = self.client.post(
            reverse("atlas:continent:change", args=[self.continent.pk]), test_data
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_country_list_view(self):  # noqa: D102
        response = self.client.get(reverse("atlas:country:index"))
        self.assertEqual(response.status_code, 200)

    def test_country_add_view(self):  # noqa: D102
        test_data = {
            "code": "UK",
            "name": "United Kingdom",
    ...
    def test_country_change_view(self):  # noqa: D102
        test_data = {
            "code": "UK",
            "name": "United Kingdom",
            "independence_day": "5/1/1707",
        ...
        response = self.client.post(
            reverse("atlas:country:change", args=[self.country.pk]), test_data
        )
        self.assertEqual(response.status_code, 302)
        models.Country.objects.get(
            pk=self.country.pk,
    ...
    def test_country_delete_view(self):  # noqa: D102
        response = self.client.post(
            reverse("atlas:country:delete", args=[self.country.pk])
        )
        self.assertEqual(response.status_code, 302)
        self.assertFalse(models.Country.objects.filter(pk=self.country.pk))

    def test_city_list_view(self):  # noqa: D102
        response = self.client.get(reverse("atlas:city:index"))
        self.assertEqual(response.status_code, 200)

    def test_city_detail_view(self):  # noqa: D102
        response = self.client.get(reverse("atlas:city:detail", args=[self.city.pk]))
        self.assertEqual(response.status_code, 200)

    def test_city_add_view(self):  # noqa: D102
        test_data = {
            "name": "Vancouver",
            "is_capital": False,
            "population": 8000000,
    ...
    def test_city_change_view(self):  # noqa: D102
        test_data = {
            "name": "Vancouver",
            "is_capital": False,
            "population": 8000000,
        ...
        response = self.client.post(
            reverse("atlas:city:change", args=[self.city.pk]), test_data
        )
        self.assertEqual(response.status_code, 302)
        models.City.objects.get(pk=self.city.pk, **test_data)

    def test_city_delete_view(self):  # noqa: D102
