...
class Test(TestCase):  # noqa: D101
    ...
    def test_admin_detail_page(self):
        review = Review.objects.create(text="Test", author=self.admin, stage=ReviewState.NEW)
        response = self.client.get('/admin/review/review/%d/change/' % review.pk)
        self.assertEqual(response.status_code, 200)

    def test_admin_default_transition_page(self):
        review = Review.objects.create(text="Test", author=self.admin, stage=ReviewState.NEW)
        response = self.client.get('/admin/review/review/%d/transition/remove/' % review.pk)
        self.assertEqual(response.status_code, 200)

    def test_admin_invalid_transition_page(self):
        review = Review.objects.create(text="Test", author=self.admin, stage=ReviewState.NEW)
        response = self.client.get('/admin/review/review/%d/transition/__invalid__/' % review.pk)
        self.assertEqual(response.status_code, 400)

    def test_admin_custom_fields_transition_page(self):
        review = Review.objects.create(text="Test", author=self.admin, stage=ReviewState.NEW)
        response = self.client.get('/admin/review/review/%d/transition/approve/' % review.pk)
        self.assertEqual(response.status_code, 200)

    def test_admin_custom_view_transition_page(self):
        review = Review.objects.create(text="Test", author=self.admin, stage=ReviewState.NEW)
        response = self.client.get('/admin/review/review/%d/transition/reject/' % review.pk)
        self.assertEqual(response.status_code, 200)
